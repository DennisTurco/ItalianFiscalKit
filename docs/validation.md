...
```csharp
CodiceFiscaleValidator.IsValid("rssmra85t10a562s");  // true  — lowercase accepted
CodiceFiscaleValidator.IsValid("RSS1RA85T10A562S");  // false — digit where a letter is expected
CodiceFiscaleValidator.IsValid("RSSMRA85110A562S");  // false — digit in month position
```
...
```csharp
CodiceFiscaleValidator.IsValid("RSSMRA85D31A562?");  // false — April only has 30 days
...
```csharp
CodiceFiscaleValidator.IsValid("RSSMRA85T10Z999S");  // false — Z999 doesn''t exist
CodiceFiscaleValidator.IsValid("MRNGRL01P55Z614X");  // true  — Z614 is Venezuela
```
